...
public class exCommController {
    ...
    public String exCommDelete(
        ...
        if (dto == null) return "redirect:/exchange/exchangeDetail?ex_no=" + ex_no;

        String loginId = principal.getName();
        boolean isAdmin = authentication.getAuthorities().stream()
                .anyMatch(a -> a.getAuthority().equals("ROLE_ADMIN")); // ⭐ 핵심

        if (isAdmin || loginId.equals(dto.getExc_writer())) {
